...
```
### Flex布局/弹性布局
- 是CSS3的一种新的浏览器提倡的布局模式；灵活控制块级盒子的布局方式，避免浮动布局脱标的问题   
...
1、内容  
- 由弹性容器（Flex container）、弹性子元素/盒子(Flex item)组成
- 主轴、侧轴（交叉轴）组成。  （方向是可改变的） 
...
- 给亲父元素添加`display:flex;`开启弹性盒，子元素沿着主轴默认水平横向排列
- 盒子之间的间距，是通过调节主轴或侧轴的对齐方式来设置
    - 修改主轴的对齐方式`justify-content`给父元素（弹性容器）添加
    ...
    - 修改侧轴的对齐方式`align-items`给父元素（弹性容器）添加  
...
- 修改主轴方向：`flex-direction`，主轴若竖着，侧轴就自动默认横着。
...
> 举例   
先确定主轴方向，再选择对应的属性实现主轴或侧轴的对齐方式

- Flex布局，默认沿着主轴排不换行，父级宽度不够，子级就自动挤压来适应父级宽度
    - 弹性盒子换行，多行排列效果 `flex-wrap:wrap;`
    ...
    - 调整行对齐方式 `align-content`
        ...
        |flex-end|终点开始依次排列|
        |`center`|居中排列|
        |`space-around`|弹性盒子空白间距，均分在弹性盒子两侧|
        |`space-between`|弹性盒子空白间距，均分在相邻盒子之间|


子元素上的属性

- `align-self`:控制某个子级盒子在侧轴的对齐方式（给子元素添加） 
