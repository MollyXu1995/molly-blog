...
### Less
一、简介
- less就是快速编译生成css代码的一个工具，`less文件`，`后缀是.less`  
- css语言的扩展，添加了基本 CSS 中不可用的功能，使css具备一定的`逻辑性、计算能力`。
- 如，完成px单位到rem单位的转换（ 如68/37.5=1.813rem）  
- `安装vscode插件：EasyLess`。
- `.less文件保存后`，会在`同级目录下自动生成一个 .css文件`
- `只编写 .less文件`，`.css文件会自动同步更新`
...
- 开发网站时，less文件中，可以使用`less导入`写法`引用其他less文件`代码
- 此导入`是把引用的文件里的所有代码导入对应的css文件里`
...
2、单独导出路径
- 必须在less文件第一行，写入代码。
- 如，将css文件保存到abc文件夹`// out: ./abc/`

- 如，将css文件保存到qqq文件夹，文件命名为lisa `// out: ./qqq/lisa.css`

- 如，将css文件保存到与less文件同级目录下 `// out: ./`
...
### Sass/Scss
一、简介    
- Sass，或“Syntactically Awesome StyleSheets”，它添加了基本 CSS 中不可用的功能，使您可以更轻松地简化和维护项目的样式表。

- Sass 扩展了 CSS3，增加了规则、变量、混入、选择器、继承、内置函数等特性。
- Sass 完全兼容所有版本的 CSS。
